let quizzes = [
  ...
  [
    ...
    },
    {
      ...
      grade: 7,
      topic: "Statistics",
      question: "Which graph is best to represent a comparison of categories?",
...
let filteredQuizzes = quizzes.filter(gradeQuizzes => {
  return gradeQuizzes[0].grade == location.pathname.split('/').pop().split('.')[0].split('-')[1]
}).flat().map(question => question.topic)
filteredQuizzes = new Set(filteredQuizzes)
